...
class TinyRecursiveReasoningModel_ACTV1_Inner(nn.Module):
    def __init__(self, config: TinyRecursiveReasoningModel_ACTV1Config) -> None:
        ...
        self.L1_init = mx.array(trunc_normal_init_(mx.zeros(self.config.hidden_size, dtype=self.forward_dtype), std=1))
        self.L2_init = mx.array(trunc_normal_init_(mx.zeros(self.config.hidden_size, dtype=self.forward_dtype), std=1))
        self.L3_init = mx.array(trunc_normal_init_(mx.zeros(self.config.hidden_size, dtype=self.forward_dtype), std=1))
        self.L4_init = mx.array(trunc_normal_init_(mx.zeros(self.config.hidden_size, dtype=self.forward_dtype), std=1))
        self.L5_init = mx.array(trunc_normal_init_(mx.zeros(self.config.hidden_size, dtype=self.forward_dtype), std=1))
        self.L6_init = mx.array(trunc_normal_init_(mx.zeros(self.config.hidden_size, dtype=self.forward_dtype), std=1))
    ...
    def empty_carry(self, batch_size: int):
        return TinyRecursiveReasoningModel_ACTV1InnerCarry(
            ...
            z_L1=mx.zeros((batch_size, self.config.seq_len + self.puzzle_emb_len, self.config.hidden_size), dtype=self.forward_dtype),
            z_L2=mx.zeros((batch_size, self.config.seq_len + self.puzzle_emb_len, self.config.hidden_size), dtype=self.forward_dtype),
            z_L3=mx.zeros((batch_size, self.config.seq_len + self.puzzle_emb_len, self.config.hidden_size), dtype=self.forward_dtype),
            z_L4=mx.zeros((batch_size, self.config.seq_len + self.puzzle_emb_len, self.config.hidden_size), dtype=self.forward_dtype),
            z_L5=mx.zeros((batch_size, self.config.seq_len + self.puzzle_emb_len, self.config.hidden_size), dtype=self.forward_dtype),
            z_L6=mx.zeros((batch_size, self.config.seq_len + self.puzzle_emb_len, self.config.hidden_size), dtype=self.forward_dtype),
        )
    ...
    def reset_carry(self, reset_flag: mx.array, carry: TinyRecursiveReasoningModel_ACTV1InnerCarry):
        return TinyRecursiveReasoningModel_ACTV1InnerCarry(
            ...
            z_L1=mx.where(reset_flag[..., None, None], self.L1_init, carry.z_L1),
            z_L2=mx.where(reset_flag[..., None, None], self.L2_init, carry.z_L2),
            z_L3=mx.where(reset_flag[..., None, None], self.L3_init, carry.z_L3),
            z_L4=mx.where(reset_flag[..., None, None], self.L4_init, carry.z_L4),
            z_L5=mx.where(reset_flag[..., None, None], self.L5_init, carry.z_L5),
            z_L6=mx.where(reset_flag[..., None, None], self.L6_init, carry.z_L6),
        )
    ...
    def __call__(self, carry: TinyRecursiveReasoningModel_ACTV1InnerCarry, batch: Dict[str, mx.array]) -> Tuple[TinyRecursiveReasoningModel_ACTV1InnerCarry, mx.array, Tuple[mx.array, mx.array]]:
        ...
        # LM Outputs
        new_carry = TinyRecursiveReasoningModel_ACTV1InnerCarry(z_H=z_H, z_L1=z_L[0], z_L2=z_L[1], z_L3=z_L[2], z_L4=z_L[3], z_L5=z_L[4], z_L6=z_L[5])  # New carry no grad
