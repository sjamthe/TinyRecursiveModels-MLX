...
class TinyRecursiveReasoningModel_ACTV1ReasoningModule(nn.Module):
    ...
    def __call__(self, hidden_states: mx.array, **kwargs) -> mx.array:
...
class TinyRecursiveReasoningModel_ACTV1_Inner(nn.Module):
    ...
    def __call__(self, carry: TinyRecursiveReasoningModel_ACTV1InnerCarry, batch: Dict[str, mx.array]) -> Tuple[TinyRecursiveReasoningModel_ACTV1InnerCarry, mx.array, Tuple[mx.array, mx.array]]:
        ...
        # LM Outputs
        new_carry = TinyRecursiveReasoningModel_ACTV1InnerCarry(z_L=z_L)  # New carry no grad
        output = self.lm_head(z_out)[:, self.puzzle_emb_len:]
        q_logits = self.q_head(z_out[:, 0]).astype(mx.float32) # Q-head; uses the first puzzle_emb position
