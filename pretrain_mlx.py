...
def create_model(config: PretrainConfig, train_metadata: PuzzleDatasetMetadata):
    ...
    if config.arch.puzzle_emb_ndim == 0:
        ...
        optimizers = [
            CastedSparseEmbeddingSignSGD_MLX(
                learning_rate=0,  # Needs to be set by scheduler
...
def save_train_state(config: PretrainConfig, train_state: TrainState):
    ...
    mx.savez(os.path.join(config.checkpoint_path, f"step_{train_state.step}"), **train_state.model.parameters())
...
def load_checkpoint(model: nn.Module, config: PretrainConfig):
    if config.load_checkpoint is not None:
        ...
        # Load state dict
        state_dict = mx.load(config.load_checkpoint)

        # Resize and reset puzzle emb if needed
        puzzle_emb_name = "puzzle_emb.weights"
        if puzzle_emb_name in state_dict:
            ...
            if puzzle_emb.shape != expected_shape:
                ...
                state_dict[puzzle_emb_name] = (
                    mx.mean(puzzle_emb, axis=0, keepdims=True).broadcast_to(expected_shape)
                )
...
def train_batch(config: PretrainConfig, train_state: TrainState, batch: Any, global_batch_size: int):
    ...
    (loss, (new_carry, metrics)), grads = mx.value_and_grad(loss_fn, has_aux=True)(train_state.model, train_state.carry, batch)
    ...
    for optim, base_lr in zip(train_state.optimizers, train_state.optimizer_lrs):
        ...
        optim.learning_rate = lr_this_step
        # Update model parameters
        train_state.model = optim.apply_gradients(train_state.model, grads)

    # Process metrics
    if len(metrics):
        metric_keys = list(sorted(metrics.keys()))  # Sort keys to guarantee consistent order
        metric_values = [metrics[k] for k in metric_keys]
        
        # Postprocess
        count = max(metrics["count"], 1)  # Avoid NaNs
        reduced_metrics = {f"train/{k}": v / (global_batch_size if k.endswith("loss") else count) for k, v in metrics.items()}
...
def evaluate(
    ...
    for set_name, batch, global_batch_size in eval_loader:
        ...
        for collection in (batch, preds):
            for k, v in collection.items():
                if k in config.eval_save_outputs:
                    ...
                    save_preds[k].append(v)  # Keep on device for MLX
        ...
        metric_values[set_id] += mx.stack([metrics[k] for k in metric_keys])
    ...
    if config.checkpoint_path is not None and len(save_preds):
        ...
        mx.savez(
            os.path.join(config.checkpoint_path, f"step_{train_state.step}_all_preds"),
            **save_preds
        )
...
def load_synced_config(hydra_config: DictConfig) -> PretrainConfig:
...
def launch(hydra_config: DictConfig):
    ...
    wandb.log({"num_params": sum(x.size for x in train_state.model.parameters().values())}, step=0)
